    ...
    while (dt <= end) {
        ...
        if (x_purok2.indexOf(iso) !== -1) {    
            ...
            data2.push({
                ...
                y: isoDayOfWeek(dt),
                d: iso,
                v: y_purok2[x_purok2.findIndex(indexNum)]
            });
        } else {
            data2.push({
                x: iso,
...
const dataPrk2 = {
    datasets: [{
        label: 'Heat Map - Purok 2',
        data: dataFromDBPurok2(),
        backgroundColor(c) {
...
// render init block
const matrixPrk2 = new Chart(
    document.getElementById('matrixChartPurok2'),
    configPrk2
);