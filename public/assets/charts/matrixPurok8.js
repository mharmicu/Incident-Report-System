    ...
    while (dt <= end) {
        ...
        if (x_purok8.indexOf(iso) !== -1) {
            ...
            data2.push({
                ...
                y: isoDayOfWeek(dt),
                d: iso,
                v: y_purok8[x_purok8.findIndex(indexNum)]
            });
        } else {
            data2.push({
                x: iso,
...
const dataPrk8 = {
    datasets: [{
        label: 'Heat Map - Purok 8',
        data: dataFromDBPurok8(),
        backgroundColor(c) {
...
// render init block
const matrixPrk8 = new Chart(
    document.getElementById('matrixChartPurok8'),
    configPrk8
);