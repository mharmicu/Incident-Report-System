    ...
    while (dt <= end) {
        ...
        if (x_stateresa.indexOf(iso) !== -1) {
            ...
            data2.push({
                ...
                y: isoDayOfWeek(dt),
                d: iso,
                v: y_stateresa[x_stateresa.findIndex(indexNum)]
            });
        } else {
            data2.push({
                x: iso,
...
const dataSta = {
    datasets: [{
        label: 'Heat Map - Sta Teresa',
        data: dataFromDBStaTeresa(),
        backgroundColor(c) {
...
// render init block
const matrixSta = new Chart(
    document.getElementById('matrixChartSta'),
    configSta
);