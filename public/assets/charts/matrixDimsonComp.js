    ...
    while (dt <= end) {
        ...
        if (x_dimsoncomp.indexOf(iso) !== -1) {
            ...
            data2.push({
                ...
                y: isoDayOfWeek(dt),
                d: iso,
                v: y_dimsoncomp[x_dimsoncomp.findIndex(indexNum)]
            });
        } else {
            data2.push({
                x: iso,
...
const dataDimson = {
    datasets: [{
        label: 'Heat Map - Dimson Compound',
        data: dataFromDBDimsonComp(),
        backgroundColor(c) {
...
// render init block
const matrixDimson = new Chart(
    document.getElementById('matrixChartDimson'),
    configDimson
);