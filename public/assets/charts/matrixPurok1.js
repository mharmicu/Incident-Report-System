    ...
    while (dt <= end) {
        ...
        if (x_purok1.indexOf(iso) !== -1) {    
            ...
            data2.push({
                ...
                y: isoDayOfWeek(dt),
                d: iso,
                v: y_purok1[x_purok1.findIndex(indexNum)]
            });
        } else {
            data2.push({
                x: iso,
...
const dataPrk1 = {
    datasets: [{
        label: 'Heat Map - Purok 1',
        data: dataFromDBPurok1(),
        backgroundColor(c) {
            const value = c.dataset.data[c.dataIndex].v
            let alpha = (5 + value) / 10;
            if (alpha > 0.5) {
                return `rgba(0, 180, 0, ${alpha})`;
            }else{
...
// render init block
const matrixPrk1 = new Chart(
    document.getElementById('matrixChartPurok1'),
    configPrk1
);