...
//testing code block 
function dataFromDBCaldozoComp() {
    const d = new Date();
    ...
    while (dt <= end) {
        ...
        if (x_caldozocomp.indexOf(iso) !== -1) {
            ...
            data2.push({
                ...
                y: isoDayOfWeek(dt),
                d: iso,
                v: y_caldozocomp[x_caldozocomp.findIndex(indexNum)]
            });
        } else {
            data2.push({
                x: iso,
...
const dataCaldozo = {
    datasets: [{
        label: 'Heat Map - Caldozo Compound',
        data: dataFromDBCaldozoComp(),
        backgroundColor(c) {
...
// render init block
const matrixCaldozo = new Chart(
    document.getElementById('matrixChartCaldozo'),
    configCaldozo
);