    ...
    while (dt <= end) {
        ...
        if (x_purok3.indexOf(iso) !== -1) {
            ...
            data2.push({
                ...
                y: isoDayOfWeek(dt),
                d: iso,
                v: y_purok3[x_purok3.findIndex(indexNum)]
            });
        } else {
            data2.push({
                x: iso,
...
const dataPrk3 = {
    datasets: [{
        label: 'Heat Map - Purok 3',
        data: dataFromDBPurok3(),
        backgroundColor(c) {
...
// render init block
const matrixPrk3 = new Chart(
    document.getElementById('matrixChartPurok3'),
    configPrk3
);