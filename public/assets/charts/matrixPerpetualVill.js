...
//testing code block 
function dataFromDBPerpetualVill() {
    const d = new Date();
    ...
    while (dt <= end) {
        ...
        if (x_perpetualvill.indexOf(iso) !== -1) {
            ...
            data2.push({
                ...
                y: isoDayOfWeek(dt),
                d: iso,
                v: y_perpetualvill[x_perpetualvill.findIndex(indexNum)]
            });
        } else {
            data2.push({
                x: iso,
...
const dataPerpetual = {
    datasets: [{
        label: 'Heat Map - Perpetual Village',
        data: dataFromDBPerpetualVill(),
        backgroundColor(c) {
...
// config 
const configPerpetual = {
    type: 'matrix',
    data: dataPerpetual,
    options: {
        maintainAspectRatio: false,
        scales: scalesPerpetual,
        plugins: {
            tooltip: {
                callbacks: {
...
// render init block
const matrixPerpetual = new Chart(
    document.getElementById('matrixChartPerpetual'),
    configPerpetual
);